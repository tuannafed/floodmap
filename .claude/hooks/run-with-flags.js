#!/usr/bin/env node
/**
 * Executes a hook script only when enabled by hook profile flags.
 *
 * Usage:
 *   node run-with-flags.js <hookId> <scriptRelativePath> [profilesCsv]
 *
 * Controls:
 * - CAW_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - CAW_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getPluginRoot() {
  if (process.env.CAW_PLUGIN_ROOT && process.env.CAW_PLUGIN_ROOT.trim()) {
    return process.env.CAW_PLUGIN_ROOT;
  }
  // hooks/ is one level under scripts/, scripts/ is one level under repo root
...
async function main() {
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...process.env,
      CAW_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      CAW_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
