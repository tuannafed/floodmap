...
import type { IconRecord } from '@/lib/icon-resolve';
import { withResolvedIcons } from '@/lib/icon-resolve';
...
// Pre-icon-resolution shape — what parseStack()/project-meta.py's project.yaml
// produce. withResolvedIcons() below turns this into the public StackItem.
...
export interface StackItem extends RawStackItem {
  // Resolved server-side (icon-resolve.ts, which imports the full `simple-icons`
  // package) so the client never has to — see overview-tab.tsx's StackIcon.
  iconSvg: IconRecord | null;
}
...
function deriveTags(stack: RawStackItem[], archetype: string): string[] {
...
export const GET: APIRoute = async () => {
  ...
  if (isDevMode(root)) {
    return new Response(
      JSON.stringify({ ...MOCK_OVERVIEW, stack: withResolvedIcons(MOCK_OVERVIEW.stack) }),
      {
        status: 200,
  ...
  // scripts/generators/project-meta.py. This is deterministic and avoids the
  ...
  if (meta) {
    const overview: ProjectOverview = {
      ...
      tags: meta.tags,
      stack: withResolvedIcons(meta.stack),
    };
