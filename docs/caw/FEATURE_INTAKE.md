...
Restate as work item
    |
    v
Find affected product docs and tasks
    |
    v
Run risk checklist
    |
...
| Spec slice | Implementing selected behavior from an accepted spec | Task plan |
| Change request | Changing, fixing, or refining accepted behavior | Task plan or direct patch |
| New initiative | Adding a larger product area that needs multiple tasks | Initiative notes plus task plans |
| Maintenance request | Changing technical, operational, or dependency behavior | Task plan, validation report, or decision |
| Harness improvement | Improving how humans and agents collaborate | Direct docs update or `scripts/caw/bin/harness-cli backlog add` |

Do not create or extend a monolithic spec by default after intake. Use product
docs, task plans, decisions, and initiative notes as the living surface.
...
- Record the intake row before implementation; tiny work skips full task-plan
  overhead, not durable task classification (intake + story add).
- Patch directly.
...
- Record or update proof status with `scripts/caw/bin/harness-cli story add` and
  `scripts/caw/bin/harness-cli task add` / `harness-cli task update`.
...
- Create a task with a full plan: `docs/caw/stories/<story-id>/plan.md` with
  `## Spec mandate` and a `runtime-smoke-test` task (see rules).
...
Task: docs/caw/stories/task-014-manager-updates-role/plan.md.
